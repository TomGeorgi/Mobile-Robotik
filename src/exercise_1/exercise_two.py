...
def backwards_kinematic(point: tuple):
    ...
    t_DB_R = ex.create_transformationmatrix_z(0, (-l / 2, 0, -h))
    t_R_DB = ex.create_transformationmatrix_z(0, (l / 2 - a / 2, 0, h))
    ...
    t_R_D = np.dot(t_R_DB, t_DB_D)

    t_D_R = np.linalg.inv(t_R_D)
    p_D = np.dot(t_R_D, [x, y, z, 1])
    ...
    print(a_tmp)
    print(c_tmp)
    b_tmp = math.sqrt(np.absolute((math.pow(l_two, 2) - math.pow(c_tmp, 2))))
    print(b_tmp)
